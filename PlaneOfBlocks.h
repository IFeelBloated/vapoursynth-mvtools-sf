...
// aligned to this (source plane can be ach aligned loads, 64 required for effective use of x264 sad on Core2) 1.9.5
...
class PlaneOfBlocks {
    ...
    bool smallestPlane;         /* say whether vectors can used predictors from a smaller plane */                /* can we use iasm code */
    ...
    float nMinCost;               /* minimum cost ( sad + mv cost ) found so far */
    ...
    float nLambda;                /* vector cost factor */
    float LSAD; // SAD limit for lambda using - Fizick
    ...
    float badSAD; // SAD threshold for more wide search
    int badrange; // wide search radius
    double planeSAD; // summary SAD of plane
    ...
    void SearchMVs(MVFrame *_pSrcFrame, MVFrame *_pRefFrame, SearchType st,
            int stp, float _lambda, float _lSAD, int _pennew, int _plevel, int *out, VECTOR *globalMVec, short * outfilebuf, int _fieldShiftCur,
            DCTClass * _DCT , float * _meanLumaChange, int _divideExtra,
            int _pzero, int _pglobal, float badSAD, int badrange, bool meander, int *vecPrev, bool tryMany);
