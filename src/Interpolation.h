 ...
 *  and one optimized for  processors.
 */
...
template <typename PixelType>
void RB2FilteredHorizontalInplace(uint8_t *pSrc8, int nSrcPitch, int nWidth, int nHeight)
{ // Filtered with 1/4, 1/2, 1/4 filter for smoothing and anti-aliasing - Fizick
...
{ // separable Filtered with 1/4, 1/2, 1/4 filter for smoothing and anti-aliasing - Fizick v.2.5.2
    ...
    RB2FilteredVertical<PixelType>(pDst, pSrc, nDstPitch, nSrcPitch, nWidth*2, nHeight); // intermediate half height
    RB2FilteredHorizontalInplace<PixelType>(pDst, nDstPitch, nWidth, nHeight); // inpace width reduction
}
...
template <typename PixelType>
void RB2BilinearFilteredHorizontalInplace(uint8_t *pSrc8, int nSrcPitch, int nWidth, int nHeight)
{ // BilinearFiltered with 1/8, 3/8, 3/8, 1/8 filter for smoothing and anti-aliasing - Fizick
    // nWidth is dst height which is reduced by 2 source width
...
{ // separable BilinearFiltered with 1/8, 3/8, 3/8, 1/8 filter for smoothing and anti-aliasing - Fizick v.2.5.2
    // assume he have enough horizontal dimension for intermediate results (double as final)
    RB2BilinearFilteredVertical<PixelType>(pDst, pSrc, nDstPitch, nSrcPitch, nWidth*2, nHeight); // intermediate half height
    RB2BilinearFilteredHorizontalInplace<PixelType>(pDst, nDstPitch, nWidth, nHeight); // inpace width reduction
}
...
template <typename PixelType>
void RB2QuadraticHorizontalInplace(uint8_t *pSrc8, int nSrcPitch, int nWidth, int nHeight)
{ // filtered Quadratic with 1/64, 9/64, 22/64, 22/64, 9/64, 1/64 filter for smoothing and anti-aliasing - Fizick
    // nWidth is dst height which is reduced by 2 source width
    PixelType *pSrc = (PixelType *)pSrc8;
    ...
    {
        int x = 0;
        float pSrc0 = (pSrc[x*2] + pSrc[x*2+1]) / 2; // store temporary
...
{ // separable filtered Quadratic with 1/64, 9/64, 22/64, 22/64, 9/64, 1/64 filter for smoothing and anti-aliasing - Fizick v.2.5.2
    // assume he have enough horizontal dimension for intermediate results (double as final)
    RB2QuadraticVertical<PixelType>(pDst, pSrc, nDstPitch, nSrcPitch, nWidth*2, nHeight); // intermediate half height
    RB2QuadraticHorizontalInplace<PixelType>(pDst, nDstPitch, nWidth, nHeight); // inpace width reduction
}
...
template <typename PixelType>
void RB2CubicHorizontalInplace(uint8_t *pSrc8, int nSrcPitch, int nWidth, int nHeight)
{ // filtered qubic with 1/32, 5/32, 10/32, 10/32, 5/32, 1/32 filter for smoothing and anti-aliasing - Fizick
    // nWidth is dst height which is reduced by 2 source width
    PixelType *pSrc = (PixelType *)pSrc8;
    ...
    {
        int x = 0;
        float pSrcw0 = (pSrc[x*2] + pSrc[x*2+1]) / 2; // store temporary
...
{ // separable filtered cubic with 1/32, 5/32, 10/32, 10/32, 5/32, 1/32 filter for smoothing and anti-aliasing - Fizick v.2.5.2
    // assume he have enough horizontal dimension for intermediate results (double as final)
    RB2CubicVertical<PixelType>(pDst, pSrc, nDstPitch, nSrcPitch, nWidth*2, nHeight); // intermediate half height
    RB2CubicHorizontalInplace<PixelType>(pDst, nDstPitch, nWidth, nHeight); // inpace width reduction
}
