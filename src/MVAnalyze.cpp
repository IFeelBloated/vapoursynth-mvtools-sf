...
typedef struct {
    ...
    float lsad; // SAD limit for lambda using - added by Fizick
    ...
    float badSAD; //  SAD threshold to make more wide search for bad vectors
...
static void VS_CC mvanalyzeCreate(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
    ...
    d.lsad = float((double)d.lsad / 255.0);
    d.badSAD = float((double)d.badSAD / 255.0);
    ...
    /*
       ...
       env->ThrowError("MAnalyze: pel has to be 1 or 2 or 4");
       */
