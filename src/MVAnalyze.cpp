...
static const VSFrameRef *VS_CC mvanalyzeGetFrame(int n, int activationReason, void **instanceData, void **frameData, VSFrameContext *frameCtx, VSCore *core, const VSAPI *vsapi) {
	MVAnalyzeData *d = (MVAnalyzeData *)* instanceData;
	if (activationReason == arInitial) {
		...
		int dst_height = 1;
		int dst_width = d->headerSize / sizeof(int) + vectorFields->GetArraySize();
		dst_width *= 4;
		...
		if (nref >= 0 && (nref < d->vi.numFrames || !d->vi.numFrames)) {
			...
			vectorFields->SearchMVs(pSrcGOF, pRefGOF, d->searchType, d->nSearchParam, d->nPelSearch, d->nLambda, d->lsad, d->pnew, d->plevel, d->global, reinterpret_cast<int*>(pDst), nullptr, fieldShift, DCTc, d->pzero, d->pglobal, d->badSAD, d->badrange, d->meander, nullptr, d->tryMany, d->searchTypeCoarse);
			if (d->divideExtra)
...
static void VS_CC mvanalyzeCreate(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
	...
	int nBlkX = (d.analysisData.nWidth - d.analysisData.nOverlapX) / (d.analysisData.nBlkSizeX - d.analysisData.nOverlapX);
	...
	int nWidth_B = (d.analysisData.nBlkSizeX - d.analysisData.nOverlapX) * nBlkX + d.analysisData.nOverlapX;
