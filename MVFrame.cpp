...
MVPlane::MVPlane(int _nWidth, int _nHeight, int _nPel, int _nHPad, int _nVPad, int _bitsPerSample)
{
    nWidth = _nWidth;
    nHeight = _nHeight;
    nPel = _nPel;
...
void MVPlane::Refine(int sharp)
{
    ...
    else if (( nPel == 4 ) && ( !isRefined )) // firstly pel2 interpolation
    {
        ...
        // now interpolate intermediate
            ...
            Average2<float>(pPlane[3], pPlane[0] + bytesPerSample, pPlane[2], nPitch, nExtendedWidth - 1, nExtendedHeight);
            Average2<float>(pPlane[11], pPlane[8] + bytesPerSample, pPlane[10], nPitch, nExtendedWidth - 1, nExtendedHeight);
            ...
            Average2<float>(pPlane[7], pPlane[4] + bytesPerSample, pPlane[6], nPitch, nExtendedWidth - 1, nExtendedHeight);
            Average2<float>(pPlane[15], pPlane[12] + bytesPerSample, pPlane[14], nPitch, nExtendedWidth - 1, nExtendedHeight);
...
MVFrame::MVFrame(int nWidth, int nHeight, int nPel, int nHPad, int nVPad, int _nMode, int _xRatioUV, int _yRatioUV, int _bitsPerSample)
{
    nMode = _nMode;
    xRatioUV = _xRatioUV;
    yRatioUV = _yRatioUV;
    ...
    if ( nMode & YPLANE )
        pYPlane = new MVPlane(nWidth, nHeight, nPel, nHPad, nVPad, bitsPerSample);
    else
        pYPlane = 0;

    if ( nMode & UPLANE )
        pUPlane = new MVPlane(nWidth / xRatioUV, nHeight / yRatioUV, nPel, nHPad / xRatioUV, nVPad / yRatioUV, bitsPerSample);
    else
        pUPlane = 0;

    if ( nMode & VPLANE )
        pVPlane = new MVPlane(nWidth / xRatioUV, nHeight / yRatioUV, nPel, nHPad / xRatioUV, nVPad / yRatioUV, bitsPerSample);
    else
        pVPlane = 0;
}
...
MVGroupOfFrames::MVGroupOfFrames(int _nLevelCount, int _nWidth, int _nHeight, int _nPel, int _nHPad, int _nVPad, int nMode, int _xRatioUV, int _yRatioUV, int _bitsPerSample)
{
    ...
    pFrames[0] = new MVFrame(nWidth, nHeight, nPel, nHPad, nVPad, nMode, xRatioUV, yRatioUV, bitsPerSample);
    for ( int i = 1; i < nLevelCount; i++ )
    {
        ...
        pFrames[i] = new MVFrame(nWidthi, nHeighti, 1, nHPad, nVPad, nMode, xRatioUV, yRatioUV, bitsPerSample);
    }
}
