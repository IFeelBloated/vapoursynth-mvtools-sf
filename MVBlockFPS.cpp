    ...
    if (mode == 0) {
        for (int h = 0; h < nBlkSizeY; h++) {
            for (int w = 0; w < nBlkSizeX; w++) {
                ...
                float mca = (pMCB_[w] * time256 + pMCF_[w] * (256 - time256)) / 256; // MC fetched average
                pDst_[w] = mca;
            }
            pDst += dst_pitch;
            pMCB += MCB_pitch;
        ...
        for (int h = 0; h < nBlkSizeY; h++) {
            for (int w = 0; w < nBlkSizeX; w++) {
                ...
                float mca = (pMCB_[w] * time256 + pMCF_[w] * (256 - time256)) / 256; // MC fetched average
                float sta = MEDIAN<PixelType>(pRef_[w], pSrc_[w], mca); // static median
                pDst_[w] = sta;
        ...
        for (int h = 0; h < nBlkSizeY; h++) {
            for (int w = 0; w < nBlkSizeX; w++) {
                ...
                float avg = (pRef_[w] * time256 + pSrc_[w] * (256 - time256)) / 256; // simple temporal non-MC average
                float dyn = MEDIAN<PixelType>(avg, pMCB_[w], pMCF_[w]); // dynamic median
                pDst_[w] = dyn;
            }
            pDst += dst_pitch;
            pMCB += MCB_pitch;
        ...
        for (int h = 0; h < nBlkSizeY; h++) {
            for (int w = 0; w < nBlkSizeX; w++) {
                ...
                float avg = (pRef_[w] * time256 + pSrc_[w] * (256 - time256) + 255) / 256; // simple temporal non-MC average
